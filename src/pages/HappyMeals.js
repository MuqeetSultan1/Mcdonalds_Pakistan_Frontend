...
import { happyMealActions, addToCart } from '../redux/action'
import { Link } from 'react-router-dom'

const HappyMeals = () => {

    const happyMealList = useSelector((state)=> state.happyMealList)

    const {happymeals,loading, error} = happyMealList
    console.log("happymeals",happymeals)
    
    const dispatch = useDispatch()

    useEffect(()=>{
        dispatch(happyMealActions())
    },[dispatch])
...
<div className='container-fluid'>
    <div className='row'>
        ...
        <div className='col-lg-10 col-md-4 col-sm-1 my-5' style={{ display: 'flex', flexWrap: 'wrap'  }}>
            {happymeals && happymeals.length>0 ?
                happymeals.map((pro =>{
                    return(
