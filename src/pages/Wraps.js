...
import { wrapsActions,addToCart } from '../redux/action'
import { Link } from 'react-router-dom'


const Wraps = () => {

    const wrapsList = useSelector((state)=> state.wrapsList)

    const {wraps,loading, error} = wrapsList
    console.log("wraps",wraps)
    
    const dispatch = useDispatch()

    useEffect(()=>{
        dispatch(wrapsActions())
    },[dispatch])
