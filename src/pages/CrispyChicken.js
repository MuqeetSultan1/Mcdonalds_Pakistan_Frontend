...
import { crispyChickenActions, addToCart} from '../redux/action'
import { useEffect } from 'react'
import { Link } from 'react-router-dom'

const CrispyChicken = () => {

    const crispyChickenList = useSelector((state)=> state.crispyChickenList)

    const {crispychicken,loading, error} = crispyChickenList
    console.log("crispychicken",crispychicken)
    
    const dispatch = useDispatch()

    useEffect(()=>{
        dispatch(crispyChickenActions())
    },[dispatch])
...
<div className='container-fluid'>
    <div className='row'>
        ...
        <div className='col-lg-10 col-md-4 col-sm-1 my-5' style={{ display: 'flex', flexWrap: 'wrap'  }}>
            {crispychicken && crispychicken.length>0 ?
                crispychicken.map((pro =>{
                    return(
                ...
                : <p>No crispy chicken available</p>
            }
        </div>
        <div className='col-lg-1 col-md-1 col-sm-1'></div>
    </div>
